...
from turbopanda.utils import belongs, difference, set_like, nonnegative
from turbopanda.str import pattern

from turbopanda.ml._default import model_types
...
def _model_selection_parameters(
    cv_results,
    model_name,
    params,
    plot=None,
    ...
    subset = cv_results.pipe(
        filter_rows_by_column, lambda z: z["model"] == model_name
    ...
    if subset.shape[0] == 1:
        ...
        prim_param = (
            params[0]
            if len(params) == 1
            else _comp_p(_mt.loc[model_name, "Primary Parameter"], prefix=prefix)
        )
        ...
        if len(params) == 1:
            ...
            plot.fill_between(
                _p, test_m + test_sd, test_m - test_sd, alpha=0.3, color=color
            )
        elif len(params) == 2:
            ...
            for line, c in zip(non_prim_uniq, colors):
                # mean, sd
                _p = pd.to_numeric(
                    subset_sorted.loc[subset_sorted[non_prim] == line, prim_param],
                    errors="ignore",
                )
                test_m = subset_sorted.loc[
                    subset_sorted[non_prim] == line, "mean_%s_score" % y_name
                ]
                test_sd = subset_sorted.loc[
                    subset_sorted[non_prim] == line, "std_%s_score" % y_name
                ]
                ...
                plot.plot(
                    ...
                    test_m,
                    "x-",
                    label=lf.format(non_prim.split("__")[-1], line),
                    color=c,
                )
                plot.fill_between(
                    _p, test_m + test_sd, test_m - test_sd, alpha=0.3, color=c
                )
...
def parameter_tune(
    cv_results, y: str = "test", arrange: str = "square", ax_size: int = 4
):
    ...
    cv_results = (
        cv_results.df_ if not isinstance(cv_results, pd.DataFrame) else cv_results
    )
    ...
    if "model" in cv_results.columns:
        ...
        for i, m in enumerate(models):
            ...
            _P = [
                ...
                if cv_results.loc[cv_results["model"] == m, p].dropna().shape[0] > 0
            ]
