...
def is_column_boolean(ser: Union[pd.Series, np.ndarray]) -> bool:
    ...
    return (ser.dtype.kind == "b") or (
        (ser.dtype.kind == "u") and (np.unique(ser).size <= 2)
    )
...
def is_column_discrete(ser: Union[pd.Series, np.ndarray]) -> bool:
    ...
    if ser.dtype.kind == "u" or ser.dtype.kind == "b":
        ...
        warnings.warn(
            "column '{}' of type `object` is passed, default classified as NOT discrete.".format(
                nombre
            ),
            UserWarning,
        )
        ...
        nombre = ser.name if isinstance(ser, pd.Series) else "array"
        warnings.warn(
            "column '{}' of type `signed int` is passed, default classified AS discrete.".format(
                nombre
            ),
            UserWarning,
        )
        ...
        raise TypeError(
            "type '{}' not recognized for column, needs implementation".format(
                ser.dtype.kind
            )
        )
...
def is_dataframe_float(df: Union[np.ndarray, pd.DataFrame]) -> bool:
    ...
    if isinstance(df, (np.ndarray, pd.Series, pd.Index)):
        ...
        raise TypeError(
            "`df` in `is_dataframe_float` must be of type [ndarray, DataFrame]"
        )
...
def is_potential_id(ser: pd.Series, thresh: float = 0.5) -> bool:
    ...
    return (
        (ser.unique().shape[0] / ser.shape[0]) > thresh
        if is_column_string(ser)
        else False
    )


def is_potential_stacker(
    ser: pd.Series, regex: str = ";|\t|,|", thresh: float = 0.1
) -> bool:
    """Determine whether ser is a stacker-like column."""
    return (
        ser.dropna().str.contains(regex).sum() > thresh
        if is_column_string(ser)
        else False
    )
